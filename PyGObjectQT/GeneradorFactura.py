...
def crearTabla(datos_tabla):
    ...
    datos_productos = [
        ["DNI", "Nombre", "Edad", "Genero", "Fallecido"],
    ]

    for i in range(len(datos_tabla)):
        datos_productos.append(datos_tabla[i])
    ...
    tabla_Productos = Table(datos_productos, colWidths=[150, 80, 80, 80])
    ...
    # Documento
    documento = SimpleDocTemplate("Copia_Factura.pdf", pagesize=A4)
    documento.build(elementosDoc)

    # Abrir el documento
    import os

    os.system("Copia_Factura.pdf")
