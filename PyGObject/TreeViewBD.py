...
class FestraPrincipal(Gtk.Window):
    def __init__(self):
        ...
        try:
            database = dbapi.connect("bdListinTelefonico.dat")
            # print(database)
        ...
        for i, nombreColumna in enumerate(columnas):
            ...
            if i == 2:
                ...
                #celda.connect("edited", self.on_celda_edited, modelo,i)   #Opcion 1
    ...
    def actualizarDatos(self, nome, apelido, telefono):
        try:
            ...
            # print(database)
            cursor = database.cursor()
            cursor.execute("update listaTelefonos set telefono = ? where nome = ? and apelidos = ?", (telefono, nome, apelido))
            database.commit()
    ...
    def on_celdaTelefono_edited(self, celda, fila, texto, modelo,columna):
        try:
            ...
            # print(database)
            cursor = database.cursor()
            cursor.execute("update listaTelefonos set telefono = ? where telefono= ?",
                           (texto, modelo[fila][2]))
            database.commit()
